...
class qt_color_sink_new : public base_sink<Mutex> {
public:
    qt_color_sink_new(QTextEdit *qt_text_edit,
                  int max_lines,
...
protected:
    ...
    void sink_it_(const details::log_msg &msg) override {
        if (qt_text_edit_->property("level").isValid()) {
            if (qt_text_edit_->property("level").toInt() != msg.level)
                return;
        }
...
using qt_color_sink_new_mt = qt_color_sink_new<std::mutex>;
using qt_color_sink_new_st = qt_color_sink_new<details::null_mutex>;
}  // namespace sinks
...
inline std::shared_ptr<logger> qt_color_logger_new_mt(const std::string &logger_name,
                                                  QTextEdit *qt_text_edit,
                                                  int max_lines,
                                                  bool is_utf8 = false) {
    return Factory::template create<sinks::qt_color_sink_new_mt>(logger_name, qt_text_edit, max_lines,
                                                             false, is_utf8);
}

template <typename Factory = spdlog::synchronous_factory>
inline std::shared_ptr<logger> qt_color_logger_new_st(const std::string &logger_name,
                                                  QTextEdit *qt_text_edit,
                                                  int max_lines,
                                                  bool is_utf8 = false) {
    return Factory::template create<sinks::qt_color_sink_new_st>(logger_name, qt_text_edit, max_lines,
                                                             false, is_utf8);
}
